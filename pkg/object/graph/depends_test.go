...
import (
	...
	dtestutil "sigs.k8s.io/cli-utils/pkg/object/dependson/testutil"
...
func TestSortObjs(t *testing.T) {
	testCases := map[string]struct {
		...
		"one object depends on the other; two single object sets": {
			objs: []*unstructured.Unstructured{
				testutil.Unstructured(t, resources["deployment"],
					dtestutil.AddDependsOn(t, testutil.ToIdentifier(t, resources["secret"]))),
				testutil.Unstructured(t, resources["secret"]),
			},
			expected: [][]*unstructured.Unstructured{
				{
		...
		"three objects depend on another; three single object sets": {
			objs: []*unstructured.Unstructured{
				testutil.Unstructured(t, resources["deployment"],
					dtestutil.AddDependsOn(t, testutil.ToIdentifier(t, resources["secret"]))),
				testutil.Unstructured(t, resources["secret"],
					dtestutil.AddDependsOn(t, testutil.ToIdentifier(t, resources["pod"]))),
		...
		"two objects depends on each other is cyclic dependency": {
			objs: []*unstructured.Unstructured{
				testutil.Unstructured(t, resources["deployment"],
					dtestutil.AddDependsOn(t, testutil.ToIdentifier(t, resources["secret"]))),
				testutil.Unstructured(t, resources["secret"],
					dtestutil.AddDependsOn(t, testutil.ToIdentifier(t, resources["deployment"]))),
			},
			expected: [][]*unstructured.Unstructured{},
			isError:  true,
		},
